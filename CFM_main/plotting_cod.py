...
class CfmPlotter():
    ...
    def __init__(self, fpath=None, f1path=None):
        self.fpath = fpath
        self.f1path = f1path
        f = h5py.File(fpath)
        f1 = h5py.File(f1path)
    ...
    def plotting(self):
        ...
        plt.savefig('results/final/T_double_ramp.pdf')
        plt.tight_layout
        plt.show()
...
plots = CfmPlotter('results/final/CFMresults_T_double_ramp_50y_500y_HLdynamic.hdf5',
                   'results/final/CFMresults_T_double_ramp_50y_500y_HLdynamic_grav.hdf5')
plots.plotting()
