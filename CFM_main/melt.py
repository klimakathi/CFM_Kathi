...
def percolation_bucket(self, iii):
    ...
    melt_boxes_LWC_vol = np.sum(self.LWC[
                                0:ind1 + 1]) - pm_lwc  # include the water mass from the volumes/nodes that melt (currently does not include from the partial melt box)
    ...
    self.dz = np.concatenate(
        (self.dz[ind1:-1], self.dz[-1] * np.ones(num_boxes_melted)))  # this adds new boxes at the bottom.
    self.dz[0] = pm_dz
    self.Dcon = np.concatenate((self.Dcon[ind1:-1], self.Dcon[-1] * np.ones(num_boxes_melted)))
    self.dzn = np.concatenate((np.zeros(num_boxes_melted - 1), self.dz[
                                                               0:]))  # this will fail in the case that there is no PM box, i.e. the melt mass is exactly equal to the mass of one or several boxes.
    ### old version:
    ...
    cold_content = CP_I * self.mass * (
                T_MELT - self.Tz)  # cold content of each box, i.e. how much heat to bring it to 273K (J)
    ...
    refreeze_mass_pot_sum = refreeze_mass_pot.cumsum(
        axis=0)  # total meltwater mass could be refrozen due to cold content (kg)

    ### calculate what the values will be after refreeze happens (pot stands for potential)
    rho_pot = (
                          self.mass + refreeze_mass_pot) / self.dz  # what the density of each node would be if it refroze the maximum possible
    ...
    Wmi = 0.057 * (
                RHO_I - rho_pot) / rho_pot + 0.017  # water per snow-plus-water mass irreducible liquid water content, Langen eqn 3 unitless)
    Swi = Wmi / (1 - Wmi) * (rho_pot * RHO_I) / (1000 * (
                RHO_I - rho_pot))  # irreducible water saturation, volume of water per porespace volume (unitless), Colbeck 1972
    ...
    overIRRinds = np.where(self.LWC > irreducible_vol_pot)[
        0]  # Indicies of nodes where the current LWC volume is more than irreducible
    ...
    maxLWC2_pot = ((917.0 * self.dz) - (
                self.mass + refreeze_mass_pot)) / RHO_W_KGM  # [m], volume; double check that the LWC does not get too large (i.e. that it could not push the density beyond ice density).
    ...
    extra_liquid_mass = np.sum(
        self.LWC[self.LWC > irreducible_vol_pot] * RHO_W_KGM - irreducible_mass_pot[self.LWC > irreducible_vol_pot])
    ...
    if np.any(self.rho[0:ind_p + 1] >= impermeable_rho):
        ...
        ind_p = np.where(self.rho >= impermeable_rho)[0][
            0]  # - 1 # the index of the node that has density greater than the impermeable density
        id1 = np.where(self.LWC > irreducible_vol_pot)[
            0]  # indices where the liquid water content is greater than the irreducible
        id2 = id1[id1 < ind_p]

        extra_liquid_mass = np.sum(self.LWC[id2] * RHO_W_KGM) - np.sum(irreducible_mass_pot[id2])
        storage_mass_pot = liquid_storage_mass_pot[0:ind_p] + refreeze_mass_pot[
                                                              0:ind_p]  # how much can be refrozen plus how much will stick around due to capillary
        storage_mass_pot_sum = storage_mass_pot.cumsum(axis=0)
        ...
        if total_liquid_mass >= np.sum(maxLWC_mass_pot_f):  # all porespace gets filled and there is runoff
            ...
            self.LWC[ind_g + 1:ind_p] = maxLWC_mass_pot[
                                        ind_g + 1:ind_p] / RHO_W_KGM  # fill the indices up with the maximum allowed water
    ...
    ### there is not an impermeable layer, water goes to layer ind_p
        ...
        self.Tz[0:ind_p] = T_MELT
        lw_mass_retained = np.sum(refreeze_mass_pot[0:ind_p]) + np.sum(irreducible_mass_pot[0:ind_p]) - np.sum(
            lwc_old[0:ind_p] * RHO_W_KGM)
...
def bucketVV(self, iii):
    ...
    melt_volume_IE = self.snowmeltSec[
                         iii] * S_PER_YEAR  # This still has to be checked by Max (division by self.c['stpsPerYear']?) [m]
    ...
    melt_boxes_LWC_vol = np.sum(self.LWC[
                                0:ind1 + 1]) - pm_lwc  # include the LWC from the boxes that melt (currently does not include from the partial melt box) [m]
    ...
    self.rho = np.concatenate((self.rho[ind1:-1], self.rho[-1] * np.ones(
        num_boxes_melted)))  # VV add at bottom of column as many layers as were melted away
    self.LWC = np.concatenate((self.LWC[ind1:-1], self.LWC[-1] * np.ones(num_boxes_melted)))
    ...
    if np.any(self.dz < 1e-6):  # VV change 09/12/2020
        self.dz = np.maximum(self.dz,
                             1e-6)  # avoids dz[0] to be extremely small if melt amount makes pm_dz very close to 0

    self.Dcon = np.concatenate((self.Dcon[ind1:-1], self.Dcon[-1] * np.ones(num_boxes_melted)))
    self.dzn = np.concatenate((np.zeros(num_boxes_melted), self.dz[
                                                           1:]))  # this is not quite right because is assumes compaction for the pm box is zero.
    self.dzn = self.dzn[0:self.compboxes]
    self.Tz = np.concatenate((self.Tz[ind1:-1], self.Tz[-1] * np.ones(num_boxes_melted)))
    ...
    if np.any(self.LWC > 0) == True:
        ...
        porosity_refr[frl] = porosity[
                                 frl] * RHO_I / RHO_W_KGM  # space available for liq water volume once refrozen, Wever 2014 (9) [/]
        porespace_refr_vol[frl] = porosity_refr[frl] * self.dz[frl]  # Available pore space of each layer [m]
        cold_content[frl] = CP_I * self.mass[frl] * (
                    T_MELT - self.Tz[frl])  # cold content of each box, i.e. how much heat to bring it to 273K [J]
        refreeze_mass_pot[frl] = cold_content[
                                     frl] / LF_I  # how much mass of the meltwater could be refrozen due to cold content [kg]
        refreeze_vol_pot[frl] = refreeze_mass_pot[
                                    frl] / 1000.  # how much meters of the meltwater could be refrozen due to cold content [m]
    ...
    if iii == 0:  # if water in initial profile, let it percolate according to bucket scheme
        ...
        irr_limit = np.maximum(irr * porespace_refr_vol,
                               0.)  # max amount of lwc that a layer can hold in its pore space
        ...
        if len(lwc_above_irr) > 0:  # if there are some layers exceeding irreducible water content
            ...
            if ((max(lwc_above_irr) < 1e-6) and (
                    sum(lwc_above_irr) < 1e-3)):  # if the lwc above irr_limit is very small (due to slight densification): cheat -> add it to runoff
                ...
                for index in perclayers[0]:
                    ...
                    while toperc > 0:
                        if self.Tz[ii] < 273.15:  # there can be some refreezing
                            p_frozen[ii] = min(refreeze_vol_pot[ii],
                                               porespace_refr_vol[ii])  # max freezing possible [mWE]
                            ...
                            self.Tz[ii] = T_MELT - cold_content[ii] / (
                                        CP_I * self.mass[ii])  # temperature is changed accordingly
                            porosity[ii] = 1 - self.rho[ii] / RHO_I  # Definition of porosity [/]
                            porespace_vol[ii] = porosity[ii] * self.dz[ii]  # Pore space of each layer [m]
                            porosity_refr[ii] = porosity[
                                                    ii] * RHO_I / RHO_W_KGM  # space available for liq water volume once refrozen, Wever 2014 (9) [/]
                            porespace_refr_vol[ii] = porosity_refr[ii] * self.dz[
                                ii]  # Available pore space of each layer [m]
                            cold_content[ii] = CP_I * self.mass[ii] * (T_MELT - self.Tz[
                                ii])  # cold content of each box, i.e. how much heat to bring it to 273K [J]
                            refreeze_mass_pot[ii] = cold_content[
                                                        ii] / LF_I  # how much mass of the meltwater could be refrozen due to cold content [kg]
                            refreeze_vol_pot[ii] = refreeze_mass_pot[
                                                       ii] / 1000.  # how much meters of the meltwater could be refrozen due to cold content [m]
                            ...
                            if (self.rho[ii] >= rhoimp):  # if we are in an impermeable layer
                                ...
                                toperc = 0.
                        if ((self.rho[ii] < rhoimp) and (irr[ii] * porespace_refr_vol[ii] - self.LWC[
                            ii]) > 0):  # water can be retained in the layer (this happens after possible refreezing)
    ...
    if tofreeze > 0:
        ...
        if (refreeze_vol_pot[0] > tofreeze) and (porespace_refr_vol[
                                                     0] > tofreeze):  # if all meltwater refreezes in layer[0] -> no need to do entire computation
            frozen[0] += tofreeze
            ipl = 0
            ...
            if np.any(self.rho >= rhoimp):  # check if there is an impermeable layer in the domain
                ipl = np.where(self.rho >= rhoimp)[0][
                    0]  # first impermeable layer -> bottom limit for downward percolation
            ...
            if percbottom == 1:
                if np.any(self.rho < rhoimp):  # check if there is an impermeable layer in the domain
                    ipl = np.where(self.rho < rhoimp)[0][
                              -1] + 1  # first impermeable layer -> bottom limit for downward percolation
                elif np.all(self.rho >= rhoimp):  # if no impermeable layer in the domain
            ...
            frcap[
                self.rho >= rhoimp] = 0  # avoids refreezing in impermeable layers if we allow water to bypass ice lenses (if percbottom==1)
            rhopot = (
                                 self.mass + frcap * 1000) / self.dz  # potential density if all the refreezing capacity was to be consumed
            ...
            retcap[0:ipl] = irr[0:ipl] * porespace_refr_volpot[0:ipl] - self.LWC[
                                                                        0:ipl]  # retention capacity of every layer after refreezing [mWE]
            retcap[
                self.rho >= rhoimp] = 0  # avoids retention in impermeable layers if we allow water to bypass ice lenses (if percbottom==1)
            if sum(frcap + retcap) >= tofreeze:  # enough retention possible to accomodate all liquid water
                ist = np.where(np.cumsum(frcap + retcap) >= tofreeze)[0][
                    0]  # layer where all water accomodated -> percolation stops there
        ...
        self.Tz[0:ipl + 1] = T_MELT - cold_content[0:ipl + 1] / (
                    CP_I * self.mass[0:ipl + 1])  # temperature is changed accordingly
    ...
    ## Sanity checks
    if abs(sum(self.LWC) + sum(self.refrozen) + self.runoff - (
            melt_volume_WE + sum(initial_lwc) + raintoadd)) > 1e-12:  # check for water balance
        print('Liquid water loss/gain, amount:',
              sum(self.LWC) + sum(self.refrozen) + self.runoff - (melt_volume_WE + sum(initial_lwc) + raintoadd))
