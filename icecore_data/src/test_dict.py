...
depth_interval, d18O_interval, ice_age_interval = get_interval_data(depth_full, d18O_full,
                                                                    ...
                                                                    time_grid_stp_,
                                                                    cop_)[:3]
d18o_smooth = smooth_data(1 / 500., d18O_interval, ice_age_interval, ice_age_interval)[0]
...
opt_dict = {'count': np.zeros([N, 1], dtype=int),
            ...
            'temperature': np.zeros([N, np.shape(ice_age_interval)[0]]),
            'ice_age': np.zeros([N, np.shape(ice_age_interval)[0]]),
            'gas_age': np.zeros([N, np.shape(ice_age_interval)[0]])}
...
def fun(theta):
    ...
    cost_func = 1 / (np.shape(temperature)[0] - 1) * np.sum((temperature - temp_interval) ** 2)
    opt_dict['a'][count] = a
    opt_dict['b'][count] = b
    opt_dict['c'][count] = c
    opt_dict['cost_function'][count] = cost_func
    opt_dict['temperature'][count, :] = temperature
    count += 1
...
if optimizer == 'minimize':
    theta_0 = [53013.175, 6612.959, 138.993]
    res_c = minimize(fun, theta_0, method='Powell')
